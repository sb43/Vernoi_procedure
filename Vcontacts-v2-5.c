...
char      planedef;              // = X, R, or B (defines how different atom radii are dealt with)
...
int    print_contacts(struct atom PDB[], int PDBtot, int atomlist[], int numatoms);
...
int main(int argc, char *argv[])
{
	...
	// calc contacts for all protein atoms
	calc_region(PDB, PDBtot, PDBlist, PDBtot);
	
	// print contacts for all protein atoms
	print_contacts(PDB, PDBtot, PDBlist, PDBtot);
...
int calc_region(struct atom PDB[], int PDBtot, int atomlist[], int numatoms)
{
	...
	double areao;    // area of sphere around atom zero
	...
	for(atomi=0; atomi<numatoms; ++atomi) {
		...
		// get atoms in contact range
		NC = get_contlist4(PDB, atomzero, contlist, PDBtot, rado, dim);
		// calculate polyhedron surrounding atomzero
		...
		SAS = areao - areatot;
		if( SAS > 0.0) {  // remove any fractional negative values (~ -0.0000002)
...
/******************************
 * subroutine print_contacts
 * created 4-Jan-2015  BJM
 ******************************/
 
// this subroutine writes the contact areas (SAS) to output for a given set of atoms.
// Here, the set of atoms is the entire protein.
...
int print_contacts(struct atom PDB[], int PDBtot, int atomlist[], int numatoms)
{
	...
	// -------------- print header ---------------
	printf("#  Atom-atom contact areas, calculated using program 'Vcontacts v2.5'\n");
	printf("#  Authors: B.J.McConkey, R.J.Najmanovich, V.Sobolev, and M.Edelman\n");
	printf("#  \n");
	if(showbonded == 'Y') {
	...
	printf("#   \n");
	printf("#  -------ATOM-------   ------CONTACT----- \n");
	printf("#  NUM NAME   RESIDUE   NUM NAME   RESIDUE   DIST   AREA%c \n",
	       (normalize == 'Y')?'%':' ');
	printf("#--------------------------------------------------------\n");
	...
	for(atomi=0; atomi<PDBtot; ++atomi) {
		...
		contnum = 1;
		printf(" %5d%5s %4d%5s %c      SAS                   %7.2f\n", PDB[atomi].atomnum,       // ADDED RJN 29.04.2008
		       PDB[atomi].atomname, PDB[atomi].resnum, PDB[atomi].res, PDB[atomi].chn, 
		...
		while(currindex != -1) {
			...
			if(showbonded == 'Y') {
				printf("                      %5d%5s %4d%5s %c %4.2f  %7.2f\n",                   // ADDED RJN 29.04.2008
				       PDB[ca_rec[currindex].atom].atomnum, PDB[ca_rec[currindex].atom].atomname,
				       PDB[ca_rec[currindex].atom].resnum, PDB[ca_rec[currindex].atom].res,
				       PDB[ca_rec[currindex].atom].chn,                                           // ADDED RJN 29.04.2008
				       ca_rec[currindex].dist, ca_rec[currindex].area);
				++contnum;
			} else {
...
RESTART:
	...
	while(1) {
		...
		if(edgeflag == 'Y') { // add edge
			...
			if(edgenum >= 200) {
				printf("WARNING: ****solution did not converge for atom %d, recalculating ***\n", atomzero);
...
void assign_radii(struct atom *PDB, long int PDBtot)
{
	...
	radfile = fopen(radfilename, "r"); //** ADDED RJN 28.04.2008
	if (radfile == NULL) {
		//printf("Can't open file %s for reading\n", filename);  //** COMMENTED OUT BY RJN 28.04.2008
		printf("Can't open file %s for reading\n", radfilename); //** ADDED BY RJN 28.04.2008
		exit(8);
	}
	while(1) {
		...
		if(oneline[0] != '#') {
			if(strncmp(&oneline[2], typename[recnum], 4)) {
				//printf("in file '%s', expected atom type %s, line %d\n", filename, typename[recnum], linenum);  //** COMMENTED OUT BY RJN 28.04.2008
				printf("in file '%s', expected atom type %s, line %d\n", radfilename, typename[recnum], linenum); //** ADDED BY RJN 28.04.2008
				exit(1);
			} else {
...
void parse_commandline(int argc, char *argv[], char FILENAME[])
{
	...
	planedef = 'R';    // default
	normalize = 'N';   // default
	strcpy(radfilename,"radii.dat"); // ADDED RJN 28.04.2008
	argi = 1;

	if (argc < 2) {
		printf("\n==== Program  Vcontacts, version 2.5 ====\n");
	...
	while(argi < argc) {
		if(!strncmp(argv[argi], "-all", 4)) {
			...
			strcpy(radfilename, argv[++argi]);    // ADDED RJN 28.04.2008
