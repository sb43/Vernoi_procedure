/*************************************************** * HEADER: Voronoi polyhedron set of programs * Vsurfaces, Vcontacts, and Vvolumes, for * calculating solvent accessible surfaces, * atom-atom contacts, and atom volumes. * An analytical solution is used for all  * calculations. ***************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>
...
#define Rw 1.4  // radius of water// ------------------------- structure definitions ----------------------
struct atom {
    ...
    int   resnum;  char  chn;          // chain id '-' for blank ADDED RJN 29.0.4.2008
...
struct contactlist {
    ...
    char    flag;    // to keep or not.
    'X' == omit.};  // ----------------- Global variables -----------------

struct atom *PDB;           // pointer to PDB array (dynamically allocated)
struct atomindex *box;      // index to PDB atoms within cubic
gridint   *PDBlist;             // list of atoms ordered by box numberstruct contactlist
contlist[100];   // list of possible atom contacts
